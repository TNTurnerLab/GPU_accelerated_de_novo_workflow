...
# use the debug flag when trying to figure
...
#  export LSF_DOCKER_VOLUMES="/scratch1/fs1/ris/application/parabricks:/INSTALL  $SCRATCH1:$SCRATCH1 $STORAGE1:$STORAGE1 $FILES:$FILES  $HOME:$HOME /storage1/fs1/tychele/Active/testing:/tychele /storage1/fs1/tychele/Active/testing/ris_testing/parabricks:/parabricks-experiment"
bsub -G compute-tychele -g /jeffrey.ng/parabrick_test -n 32 -M 196GB -R 'rusage[mem=196GB] span[hosts=1]' \
  ...
  -a 'docker(registry.gsc.wustl.edu/cpohl/parabricks:v3.0.0.6)' /parabricks/run_pipeline.py \
  deepvariant \
  ...
  --out-variants /parabricks-experiment/NA12878_extras/$name.dv.g.vcf.gz \
